...
public class CourseArray {
    ...
    public int[] getTimeSlot(int timeslotIndex) {
        int[] slotRepresentation = new int[elements.length];
        for (int i = 0; i < elements.length; i++) {
            slotRepresentation[i] = elements[i].mySlot == timeslotIndex ? 1 : -1;
        }
        return slotRepresentation;
    }

    public int[] getClashes() {
        int[] clashes = new int[elements.length];
        for (int i = 1; i < elements.length; i++) {
            clashes[i] = elements[i].clashSize();
        }
        return clashes;
    }
}
